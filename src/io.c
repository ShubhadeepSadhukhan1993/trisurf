...
ts_bool parse_args(int argc, char **argv)
{
    ...
    if (command_line_args.path[strlen(command_line_args.path) - 1] != '/')
        strcat(command_line_args.path, "/");
...
ts_bool write_vertex_xml_file(ts_vesicle *vesicle, ts_uint timestepno, ts_cluster_list *cstlist)
{
    ...
    fprintf(fh, "\n</DataArray>\n<DataArray type=\"Float64\" Name=\"shear force\" NumberOfComponents=\"3\" format=\"ascii\">\n");
    for (i = 0; i < vlist->n; i++)
    {
    ...
    for (j = i + 1; j < i + 3 * (vesicle->tlist->n); j += 3)
    { //let's continue counting from where we left of
        fprintf(fh, "%u ", j);
    }
    fprintf(fh, "\n");
...
ts_tape *parsetapebuffer(char *buffer)
{
    ...
    cfg_opt_t opts[] = {
        ...
        /* Shubhadeep: parameters for shear */
        CFG_SIMPLE_FLOAT("a", &tape->a),
        CFG_SIMPLE_FLOAT("b", &tape->b),
        CFG_SIMPLE_INT("shear_switch", &tape->shear_switch),
        CFG_SIMPLE_FLOAT("nx", &tape->nx),
        CFG_SIMPLE_FLOAT("ny", &tape->ny),
        /* Shubhadeep noise switch */
...
ts_bool getcmdline_tape(cfg_t *cfg, char *opts)
{
    ...
    for (i = 0;; i++, commands = NULL)
    {
        ...
        if (j != 2)
            fatal("Error. Command line tape options are not formatted properly", 1);
...
ts_bool cmdline_to_tape(cfg_t *cfg, char *key, char *val)
{
    ...
    if (cfg_opt == NULL)
        fatal("Commandline tape option not recognised", 1); //return TS_FAIL;
    switch (cfg_opt->type)
    {
    case CFGT_INT:
