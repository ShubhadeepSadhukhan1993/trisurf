...
ts_bool centermass(ts_vesicle *vesicle){
	...
	//center mass for x component does not  change for cylyndrical substrate along y-axis
	if(vesicle->tape->type_of_adhesion_model==4){
		vesicle->cm[0]=0;
	}

	//center mass for y component does not  change for cylyndrical substrate along x-axis
...
//move nucleus for the same vector as we moved vesicle
    ...
    vesicle->cm[0]=0.0;
    vesicle->cm[1]=0.0;
	if(vesicle->tape->plane_confinement_switch || vesicle->tape->adhesion_switch ){
